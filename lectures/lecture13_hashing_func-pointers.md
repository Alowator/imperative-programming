# lecture13_hashing_func-pointers
...
Например, если h(“hello”) = 735, то у ключа “hello” будет хеш 735.
> 
...
h(x[0]), h(x[1]), h(x[2]), …, h(x[n-1]) должны распределяться по диапазону {0, …, m-1} примерно равномерно, и “как бы случайно”.
...
Будем хранить элемент с ключом k в ячейке массива под номером h(k).
...
Ситуация, когда ключи различны (a != b), но хеши совпадают (h(a) = h(b)), называется “коллизией”.
...
На самом деле, даже если не хотеть, почти наверняка они всё равно встретятся.
...
Таким образом, при реализации хеш-таблице необходимо обрабатывать коллизии.
Есть две известных стратегии разрешения коллизий.

1. Разрешение коллизий цепочками (separate chaining).
...
Это может случиться на практике при использования очень плохой хеш-функции. Например, когда h(k) = 0 для любого ключа k
...
h(A) = 2 h(P) = 2 h(Q) = 3 h(X) = 4
h(T) = 9 h(Y) = 9
h(W) = 13 h(Y) = 15

При выполнении любой операции сначала вычисляется хеш-функция j = h(k).
...
Если в j-ой ячейке таблицы лежит элемент с ключом k и h(k) = i, то все ячейки в отрезке от i-ой до j-ой должны быть заняты.
...
Почему в таблице смесь сложений и XOR-ов? — пока этот вопрос лучше отложим.
...
1. Хеш-функция Кнута (ключи = целые числа).
...
> 
1. Линейная хеш-функция по модулю (целые числа).
...
2. Линейная комбинация (последовательности/строки).
x = <x_0, x_1, …, x_{l-1}> — ключ, который является последовательностью длины l
h(x) = [( sum a_i * x_i ) mod p] mod m
Здесь:
