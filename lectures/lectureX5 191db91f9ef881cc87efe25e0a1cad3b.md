...
**VA_ARGS** — здесь вставляются все дополнительные аргументы через запятую

Использование:
LOG(RENDERER, WARNING, “Cannot load material %s”, name);

Замечание: есть проблема, если вместо … передано ноль параметров.
#define LOG(format, …) fprintf(format, **VA_ARGS**)
LOG(“%d + %d = %d”, 1, 2, 3); -> fprintf(“%d + %d = %d”, 1, 2, 3); //OK
LOG(“hello”); -> fprintf(“hello”, ); //error
Этот случай работает по-разному на MSVC и GCC: MSVC запятую удаляет, GCC нет (согласно стандарту C).
...
char *readToken() {
while text[pos] is space: //пропускаем пробельные символы ’ ‘,’‘,’, и т.п.
