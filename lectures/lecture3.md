...
Чтобы был более понятен следующий пример, осознайте такой факт:
> В языке C часто используется такой подход к обработке ошибок,
> когда функция, в которой ошибка может произойти, использует return value
> для того, чтобы сообщить код случившейся ошибки вызвавшей её функции.
> Нулевой код говорит, что ошибки не случилсь,
> остальные коды ассоциированы с различными типами ошибок.
>
> Выглядит это так условно:
> int f1() {
> if ( случилась ошибка 1 )
> return 1;
> if ( случилась ошибка 2 )
> return 2;
> return 0;
> }
>
...
Пример случая, когда может потребоваться указатель на указатель:

int x[] = {1, 2, 3, 4, 5}; // Пусть есть глобальный массив целых чисел x.

int get_pointer(int **pp) // Функция get_pointer должна отдать нам значение указателя,
// но ещё, если что-то в ней пойдёт не так, она должна сообщить нам код ошибки.
// Код ошибки она отдаёт через return value, поэтому она возвращает int.
//
// Тогда как получить от неё указатель?
//
// Как раз для этого мы даём ей адрес того указателя, в который она запишет значение.
// Адрес указателя == указатель на указатель.
//
// Поэтому аргумент pp, который принимает эта функция,
// имеет тип указатель-на-указатель-на-int.
{
*pp = x; // Разыменовывая указатель-на-указатель-на-int pp, мы обращаемся к той ячейке памяти, на которую он указывает.
// Эта ячейка содержит значение типа указатель-на-int.
// Выполняя такое присваивание, мы в эту ячейку запишем адрес первого элемента массива x
// (адрес элемена массива - это как раз указатель-на-int).

```
  return 0; // Нулевой код возврата свидетельствует о том, что ошибок во время работы функции не произошло.
```
...
int main()
{
int* p; // Есть переменная, которая может содержать адрес целого числа.

```
  int ret = get_pointer(&p); // Вызывая get_pointer, мы хотим изменить значение переменной p.
                             // Поэтому берём адрес этой переменной и даём его этой функции в качестве аргумента.
                             // Код ошибки, который get_pointer вернёт, мы собираемся проверить, поэтому сохраним его в переменную ret.

  if (ret != 0) { // Если ошибка случилась, то должны её обработать.
      return 1;   // В данном случае мы просто завершим работу программы.
  }

  printf("%d\n", *p); // Используем полученное значение переменной p. Например, тут мы разыменовываем p,
                      // получая значение, на которое она указывает.
  return 0;
```
