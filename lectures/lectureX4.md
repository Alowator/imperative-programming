...
if (!(cond))  
AssertionFailed(#cond, **FILE**, **LINE**);
...
Обычно выводит что-то вроде: Assertion failed [sizeof(int) == 8] at C:.c:45
5) **FILE** и **LINE** — специальные встроенные макросы, которые заменяются на имя текущего файла и текущую строку в нём.
...
cl my.c /I ....//добавить относительный путь
В проекте MSVC:
...
1. Используя нестандартную директиву (работает в большинстве компиляторов):
== sum.h ==
#pragma once
// … (содержимое хедера)
...
// Список параметров, описан как *(тип, имя); впоследствии каждое поле мы будем обрабатывать определяя макрос* (Type, field):
