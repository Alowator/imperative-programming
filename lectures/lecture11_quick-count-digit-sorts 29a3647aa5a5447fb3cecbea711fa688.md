# lecture11_quick-count-digit-sorts
...
Соотношение размеров половин зависит от стратегии выбора опорного элемента:
1) Первый или последний элемент берётся как опорный.
Тогда на уже отсортированном массиве будет работать O(N^2) времени — сильно медленно.
2) Средний элемент берётся как опорный.
Будет работать квадратичное время на специально сгенерированном antiqsort-тесте.
При любой детерминированной стратегии выбора опорного элемента можно сгенерировать такой тест.
3) Выбираем в качестве опорного случайный элемент массива.
Получается время работы O(N log N) в среднем на любом массиве без одинаковых элементов (докажем это далее).
...
Если значения, равные опорному, кидать всегда в одну сторону, тогда будет зависание или O(N^2) время работы.
Хорошие решения проблемы:
1) Кидать элементы, равные опорному, по очереди влево и вправо.
2) Если элемент равен опорному, то выбирать его половину случайным образом.
Тогда даже если много элементов, равных опорному, они будут раскидываться примерно поровну.
Важный момент: нужно гарантировать, что с каждой стороны есть хотя бы один элемент!

Выводы:
1) время работы O(N^2) в худшем случае
2) опорный элемент лучше выбирать случайным образом
равные элементы надо раскидывать примерно поровну
тогда: время работы O(N log N) в среднем

> Пусть все элементы различные, а опорный выбирается из них случайным образом.
...
Шаг индукции?
Заметим, что опорный элемент выбирается N способами, все способы равновероятны.
Если опорный элемент k-ый по величине, то в левую половину попадает k элементов, а в правую — (N-k) элементов.
Исключение: когда опорный элемент максимальный, алгоритм должен запихнуть его значение влево — тогда будет деление N-1 : 1.
...
… //если опорный элемент k-ый по величине
T(N-2) + T(2) +
T(N-1) + T(1) +
...
Если это не так, то можно легко подогнать входной массив с различными элементами, на котором правильным ответом будет отсутствующая перестановка,
и алгоритм выведет что-то другое.
...
Каждый элемент просматривается ровно два раза, однако неправильно будет говорить, что алгоритм работает за O(N).
...
Не надо заводить набор списков L[*], нужно сразу определить, куда в Ans попадают элементы с ключом x, и затем аккуратно их раскидать.
