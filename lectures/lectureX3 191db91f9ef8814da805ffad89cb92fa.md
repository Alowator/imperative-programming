...
sayHello();
sayHello();
printf(“%d”, sayHelloCnt); // Глобальная переменная sayHelloCnt, как вы можете заметить, тоже не определена в main.c. Но она объявляна в hello.h.
return 0;
}
...
а также есть ссылки на неопределённые символы printf, sayHello и sayHelloCnt (и ещё на некий *GLOBAL_OFFSET_TABLE*).
$ nm hello.o
U *GLOBAL_OFFSET_TABLE*
U puts
...
и кроме того тут есть ссылка на неопределённый символ puts (и также на *GLOBAL_OFFSET_TABLE*).
