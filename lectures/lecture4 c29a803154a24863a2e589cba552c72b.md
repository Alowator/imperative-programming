...
Примеры:
char *hello = “Hello, World!”; //в hello запоминается адрес строки, сама строка хранится в некотором специальном месте
...
Хороший способ – в формате указать максимальную длину читаемой строки: scanf(“%10s”, hello);
Это единственный способ не получить срыв буфера при чтении scanf’ом произвольной введённой пользователем строки.

Альтернатива:
gets(hello); //читает полностью всё до следующего перевода строки в hello (НЕ пропускает пробелы)

Запись (printf):
printf(строка_формата, какие значения подставлять в строку);
Пример удобного форматированного вывода
printf(“Sum of %d and %d is %d”, a, b, a + b);

Чтение (scanf):
int cnt = scanf(“%d%d%d”, &a, &b, &r);
В строку формата пишем, что надо прочитать, далее адреса переменных, куда это надо сохранить.
...
char buff[256];
sprintf(buff, “%d + %d = %d”, a, b, a + b); //записать в buff
