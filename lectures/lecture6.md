...
void *malloc(size_t bytes); //size_t - это специальный целочисленный тип для задания размера буфера в памяти.
...
После удаления, эти байты памяти могут быть переиспользованы для будущих malloc (или хранить внутренние данные) — пользоваться блоков больше нельзя!
...
2) При удалении в free нужно передавать адрес именно начала блока, указатель на середину передавать нельзя.
3) malloc(0) — писать можно, может вернуть 0 или не 0; в любом случае, нужно его освобождать через free.
4) free(NULL) — писать можно, гарантированно ничего не делает.
Типичные ошибки:
...
printf(“%d”, sizeof(Pair)); //скорее всего напечатает 16
...
printf(“%d %d”, sizeof(arr), sz(arr)); //400 и 8
...
> 
1. Массив узлов и номер следующего в качестве ссылки:
typedef struct Node {
...
2. Массив узлов, в качестве ссылки используется указатель:
typedef struct Node {
double value; //значение узла
...
3. Узлы выделяются отдельными блоками в куче, ссылка = указатель:
typedef struct Node {
double value; //значение узла
struct Node *next; //указатель на следующий элемент (или 0 если его нет)
} Node;
...
(здесь должна быть картинка с узлами и стрелочками-ссылками)
Нюансы:
    1. нельзя удалить заданный узел, т.к. нужно изменить next предыдущего узла, а добраться до него нельзя
...
Из минусов: больше всяких ссылок, а значит больше возможностей ошибиться. Также больше накладные расходы памяти.
2. (цепочка) / кольцевой
...
3. (обычный) / со вспомогательным узлом (sentinel)
...
free(ptr); // если требуется освободить память, использовавшуюся для хранения элемента
}
