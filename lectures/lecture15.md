...
struct Node *l,* r; //указатели на левого и правого сыновей
};
...
2) Insert(X) — добавить элемент X в дерево (обычно равные ключи в дереве поиска запрещаются)
...
> 
1. Erase(K) — найти вершину с ключом K и удалить её
...
Умея выполнять операции Find, Insert, Erase, дерево поиска сравнивается по возможностям с хеш-таблицей.

1. LowerBound(K) — найти вершину с минимальным ключом, который больше или равен K.
...
1. Next(v): найти следующую по порядку вершину после вершины v.
...
Высоту поддеревьев нужно обновлять в процессе перестраивания дерева.
...
***#***#***#*** Hl вершина u переезжает на место вершины v.
# #
...
_**#**_#**#_ Hl + 1 вершина w поднимается вверх на место v, вершины v и u помещаются слева/справа
? ?**_***?***?____ Hl + 2
