# lecture12_linear-structures-libs-rand
...
(в языках с GC (сборкой мусора), таких как C#/Java, блоки памяти могут перемещаться с починкой всех “указателей”)
...
1. Экспоненциальная схема реаллокации (хорошо).
...
Выход: амортизированное/учётное время работы (amortized time).
...
Примечание: амортизированное время не подходит для систем с жёсткими real-time требованиями
из-за того, что хотя в среднем операции и работают быстро, но иногда всё же медленно.
...
Во многих задачах это K легко оценить, в таких случаях данный способ подходит для того, чтобы реализовать очередь.

Пример: в задаче про игру на всесибирской олимпиаде 2019 года программа игрока и игра общались через пайп.
Каждый кадр “игра” высылала программе игрока даныне об игровом мире через пайп (примерно 1 МБ) — 60 кадров в секунду.
Если очередь в таком пайпе делать как описано выше, то размер массива будет постоянно расти, через 5 минут нужно будет целых 18 ГБ памяти.
В этом случае через очередь прокачивается огромный объём данных, хотя “живых” элементов в ней всегда мало.

1. Кольцевой буфер (в принципе, тоже на массиве).
